...
/**
 * Income − expense − net savings contribution, per calendar month, for the
 * last `months` months — the same formula `getPeriodSummary`'s `netMinor`
 * uses for a single range, just grouped by month instead of run once.
 * Nothing else in the app computes this as a series; added for Home's
 * Surplus stat card sparkline.
 */
export async function getSurplusTrend(months = 6, reference: Date = new Date()): Promise<SurplusPoint[]> {
  const db = await getDb();
  const currency = await getDefaultCurrency();
  const start = new Date(reference.getFullYear(), reference.getMonth() - (months - 1), 1);
  const startIso = toIso(start);

  const flowRows = await db.getAllAsync<{ ym: string; type: string; total: number }>(
    `SELECT strftime('%Y-%m', t.date) as ym, t.type as type, SUM(t.amount_minor) as total
     FROM transactions t
     JOIN accounts a ON a.id = t.account_id
     WHERE t.type IN ('income', 'expense') AND a.currency = ? AND t.date >= ?
  ...
  const savingsInRows = await db.getAllAsync<{ ym: string; total: number }>(
    `SELECT strftime('%Y-%m', t.date) as ym, SUM(t.amount_minor) as total
     FROM transactions t
     JOIN accounts a ON a.id = t.to_account_id
  ...
  const savingsOutRows = await db.getAllAsync<{ ym: string; total: number }>(
    `SELECT strftime('%Y-%m', t.date) as ym, SUM(t.amount_minor) as total
     FROM transactions t
     JOIN accounts a ON a.id = t.account_id
     WHERE t.type = 'transfer' AND a.type = 'savings' AND a.currency = ? AND t.date >= ?
  ...
  const income = new Map(flowRows.filter((r) => r.type === 'income').map((r) => [r.ym, r.total]));
  const expense = new Map(flowRows.filter((r) => r.type === 'expense').map((r) => [r.ym, r.total]));
  const savingsIn = new Map(savingsInRows.map((r) => [r.ym, r.total]));
  const savingsOut = new Map(savingsOutRows.map((r) => [r.ym, r.total]));

  const points: SurplusPoint[] = [];
  for (let i = months - 1; i >= 0; i--) {
    const d = new Date(reference.getFullYear(), reference.getMonth() - i, 1);
    const ym = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
    const savingsContribution = (savingsIn.get(ym) ?? 0) - (savingsOut.get(ym) ?? 0);
    points.push({
      label: d.toLocaleDateString(undefined, { month: 'short' }),
      surplusMinor: (income.get(ym) ?? 0) - (expense.get(ym) ?? 0) - savingsContribution,
    });
  }
  return points;
}
